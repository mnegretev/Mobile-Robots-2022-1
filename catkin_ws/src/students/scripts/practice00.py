...
def main():
    ...
    rospy.init_node("practice00") #Es para iniciar el nodo principal de ROS
    rospy.Subscriber("/scan", LaserScan, callback_scan) #Es para crear el nodo subscritor, en ella se pasa el nombre del topico al que me quiero suscribir, el tipo, callback es uan funcion que se ejecuta por cada lectura que va llegar generando una notificacion.
    pub_cmd_vel = rospy.Publisher("/cmd_vel", Twist, queue_size=10) #Twist velocidad en el espacio
    loop = rospy.Rate(10) #Variable en Hz
	...
	pub_cmd_vel.publish(msg)
        loop.sleep() #Es un retrazo para el procesador pero especificamente al de 10Hz aproximadamente tardaria 10ms
