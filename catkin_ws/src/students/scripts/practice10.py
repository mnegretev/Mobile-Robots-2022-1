...
def forward_kinematics(q, Ti, Wi):
    ...
    for qi in len(q):   #for all qi in q:
...
def inverse_kinematics_xyzrpy(x, y, z, roll, pitch, yaw, Ti, Wi):
    ...
    #
    p = forward_kinematics(q, Ti, Wi) #llama la funcion para calculo de cinematica
    error=p-pd           #Calculo del error P-Pd
    ...
    while numpy.linalg.norm(error)>tolerance and iterations<max_iterations:  
        ...
        J = jacobian(q,Ti,Wi)  #Calculo de jacobiano
    ...
    if(iterations<max_iterations):  #return q si no se exeden las maximas iteraciones
        return q
    else:   #de lo contrario regrezar none
    return None
