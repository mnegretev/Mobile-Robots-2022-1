...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    #
    atrac=1
    force_x=(atrac/math.sqrt((robot_x-goal_x)**2+(robot_y-goal_y)**2))*(robot_x-goal_x)
    force_y=(atrac/math.sqrt((robot_x-goal_x)**2+(robot_y-goal_y)**2))*(robot_y-goal_y)
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for [distance,angle] in laser_readings:
      ...
      if distance<d0:
        fuerza=beta*math.sqrt(1/distance-1/d0)

      else:
        fuerza=0

      force_x+= fuerza * math.cos(angle+robot_a)
      force_y+= fuerza * math.sin(angle+robot_a)
...
def callback_pot_fields_goal(msg):
    ...
    print "Moviendose al punto meta " + str([goal_x, goal_y])    
    loop = rospy.Rate(20)
    ...
    while distance_to_goal>tol or rospy.is_shutdown():
    ...
    print("Ha llegado al punto meta")
