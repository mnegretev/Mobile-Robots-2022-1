...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    force_y=alfa*(robot_y-goal_y)/mag               #Uart(x,y)=alfa ((q-qg)/|q-qg|)
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    for [distance,angle] in laser_readings:
      if distance<d0:
        ...
        magnitud=0
      force_x+= magnitud * numpy.cos(angle+robot_a)
      force_y+= magnitud * numpy.sin(angle+robot_a)
...
def callback_pot_fields_goal(msg):
    ...
    while distance_to_goal>tolerance or rospy.is_shutdown():
