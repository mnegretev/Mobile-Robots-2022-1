...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    magn=math.sqrt((goal_x-robot_x)**2+(goal_y-robot_y)**2)
    force_x=alpha*(robot_x-goal_x)/magn
    force_y=alpha*(robot_y-goal_y)/magn
...
def callback_pot_fields_goal(msg):
    ...
    while dist>tol and not rospy.is_shutdown():
