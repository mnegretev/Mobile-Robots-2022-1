...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    # Error
    error = (math.atan2(goal_y - robot_y, goal_x - robot_x)) - robot_a
    if(error > math.pi):
    ...
    # Ley de control
    v = v_max * math.exp(-error*error/a)
    w = w_max * (2/(1 + math.exp(-error/b)) - 1)
...
def callback_pot_fields_goal(msg):
    ...
    epsilon=0.5
    tolerance=0.1
    robot_x, robot_y, robot_a = get_robot_pose(listener) # Posicion inicial
    dist_to_goal=math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2) # Distancia
    while dist_to_goal>tolerance or rospy.is_shutdown():
        ...
        [fx,fy]=[fax+frx,fay+fry] # Fuerza final
        [px,py]=[robot_x-epsilon*fx,robot_y-epsilon*fy] # Siguiente punto 
        
	msg_cmd_vel=calculate_control(robot_x,robot_y,robot_a,px,py) # Calculo de control
        pub_cmd_vel.publish(msg_cmd_vel) # Se publica el resultado del calculo de control
        draw_force_markers(robot_x, robot_y, fax, fay, frx, fry, fx, fy, pub_markers) # Representacion visual de fuerzas
        
	loop.sleep() # En espera
        robot_x, robot_y, robot_a = get_robot_pose(listener) # Nueva posicion del robot
        dist_to_goal=math.sqrt((goal_x - robot_x)**2 + (goal_y - robot_y)**2) # Representacion visual de fuerzas
