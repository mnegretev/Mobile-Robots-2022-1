...
def attraction_force(robot_x, robot_y, goal_x, goal_y):
    ...
    force_x=a*(robot_x-goal_x)/magnitud
    force_y=a*(robot_y-goal_y)/magnitud
...
def rejection_force(robot_x, robot_y, robot_a, laser_readings):
    ...
    [force_x, force_y] = [0,0]
    for[distancia, angulo] in laser_readings:
    	if distancia < d0:
		magnitud = beta * math.sqrt(1/distancia - 1/d0)
	else:
		magnitud = 0
	force_x += magnitud * math.cos(angulo + robot_a)
	force_y += magnitud * math.sin(angulo + robot_a)
...
def callback_pot_fields_goal(msg):
    ...
    dis_to_goal = math.sqrt((goal_x-robot_x)**2 + (goal_y-robot_y)**2)
    while dis_to_goal > tolerancia or rospy.is_shutdown():
