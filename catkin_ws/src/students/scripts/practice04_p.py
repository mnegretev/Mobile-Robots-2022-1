...
def calculate_control(robot_x, robot_y, robot_a, goal_x, goal_y):
    ...
    # Valores proporcionados por el profesor de velocidad lineal y angular de 0.3 y 0.5
    v_max=0.3
    w_max=0.5
    alpha=0.2
    beta=0.2
...
def follow_path(path):
    ...
    [my_x,my_y]=path[idx]#Asigna el primer punto como nuestras coordenadas locales
    [x_definitive, y_definitive]=path[-1]#Asigna la meta
    [robot_x, robot_y, robot_ang]=get_robot_pose(listener)#Se obtienen las coordenadas del robot
    global_error=math.sqrt((x_definitive-robot_x)*2+(y_definitive-robot_y)**2)#Se clacula el error global
    local_error=math.sqrt((my_x-robot_x)*2+(my_y-robot_y)**2)#Se calcula el error del punto al robot
    while global_error>0.1 and not rospy.is_shutdown():
        pub_cmd_vel.publish(calculate_control(robot_x, robot_y, robot_ang,my_x, my_y))#Publica velocidad calculada
        loop.sleep()
        [robot_x, robot_y, robot_ang]=get_robot_pose(listener)#Se calcula nueva posicion
        local_error=math.sqrt((my_x-robot_x)*2+(my_y-robot_y)**2)#Calculamos error al siguiente punto
        if(local_error<0.3):
            ...
            [my_x,my_y]=path[idx]#nuevos punto 
        global_error=math.sqrt((x_definitive-robot_x)*2+(y_definitive-robot_y)**2)#Error global
...
def get_robot_pose(listener):
    try:
        ([x, y, z], rotacion) = listener.lookupTransform('map', 'base_link', rospy.Time(0))
        a = 2*math.atan2(rotacion[2], rotacion[3])
