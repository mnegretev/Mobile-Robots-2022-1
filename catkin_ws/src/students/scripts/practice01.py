...
def get_inflated_map(static_map, inflation_cells):
    ...
    for i in range(height):
        for j in range(width):
            if static_map[i,j] > 0:
                for k_1 in range(i-inflation_cells, i+inflation_cells+1):
                    for k_2 in range(j-inflation_cells,j+inflation_cells+1):
                        inflated[k_1,k_2]=static_map[i,j]
...
def get_cost_map(static_map, cost_radius):
    ...
    for i in range(height):
        for j in range(width):
            if static_map[i,j] >0:
                for k_1 in range(-cost_radius,cost_radius+1):
                    for k_2 in range(-cost_radius, cost_radius+1):
                        costo = cost_radius - max(abs(k_1),abs(k_2))
                        if costo > cost_map[i+k_1,j+k_2]:
                            cost_map[i+k_1,j+k_2]=costo
    return cost_map
