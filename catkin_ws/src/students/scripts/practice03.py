...
def smooth_path(Q, alpha, beta):
    ...
    nabla[0] = 0
    nabla [len(P)-1] = 0
    while |nabla| > tol and steps < 100000:
    	for i = [1, len(nabla)-1]:
	    [xi,yi] = nabla[i]
            [xp,yp] = nabla[i-1]
            [xn,yn] = nabla[i+1]
            [xo,yo] = P[i]
            nx = alpha*(2*xi - xp -xn) + beta*(xi - xo)
            ny = alpha*(2*yi - yp -yn) + beta*(yi - yo)
            [xi, yi] = [xi - epsilon*nx, yi - epsilon*ny]
            nabla[i] = [xi,yi] 			
	    steps += 1
        P = P - nabla 
    return P
