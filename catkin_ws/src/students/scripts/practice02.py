...
def a_star(start_r, start_c, goal_r, goal_c, grid_map, cost_map):
    ...
    path = []
	#Esta parte se aumenta al codigo de classroom
	#Se comprobo que la ruta existe o no, ahora probar el nodo
    while [previous[row,col][0],previous[row,col][1]]!=[-1,-1]:	
	#Si el nodo no es nulo (se encuentra entre -1 y 1), procede
	path.insert(0,[row,col])
	#Se indica que partimos de un conjunto vacio que es el inicio
	[row,col]=previous[row,col]
	#Si se cumple agrega el nodo deseado a la ruta 
    return path
