...
#define NOMBRE "VALDESPINO MENDIETA"
...
int random_choice(std::vector<float>& weights)
{
     ...
     */
    float rand=rnd.uniformReal(0,1);
    for (size_t i=0;i<weights.size();i++)
    {
        if(rand<weights[i]){
          return i;
...
geometry_msgs::PoseArray resample_particles(geometry_msgs::PoseArray& particles, std::vector<float>& weights)
{
    ...
    for (size_t i=0;i<particles.poses.size();i++){
        int rdx=random_choice(weights);

        resampled_particles.poses[i]=particles.poses[rdx];
        ...
        float ang=atan2(particles.poses[rdx].orientation.z,particles.poses[rdx].orientation.w)*2;
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
     ...
     */
    for(int i=0; i<particles.poses.size();i++){
      float ang= atan2(particles.poses[i].orientation.z,particles.poses[i].orientation.w)*2;
      particles.poses[i].position.x+= delta_x*cos(ang)-delta_y*sin(ang)+ rnd.gaussian(0,MOVEMENT_NOISE);
      particles.poses[i].position.y+= delta_x*sin(ang)+delta_y*cos(ang)+ rnd.gaussian(0,MOVEMENT_NOISE);
      
      ang += delta_t+ rnd.gaussian(0,MOVEMENT_NOISE);
