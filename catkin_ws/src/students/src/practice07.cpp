...
geometry_msgs::PoseArray resample_particles(geometry_msgs::PoseArray& particles, std::vector<float>& weights)
{
     ...
     */
        for(size_t i=0; i< particles.poses.size(); i++){
        int eleccion=random_choice(weights);
        resampled_particles.poses[i]=particles.poses[eleccion];
        ...
        float angle=atan2(particles.poses[eleccion].orientation.z,particles.poses[eleccion].orientation.w)*2;
