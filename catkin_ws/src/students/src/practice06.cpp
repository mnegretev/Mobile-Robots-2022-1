...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
     ...
     */
     for (int i=0; i< N;i++)
	{
	  particles.poses[i].position.x = rnd.uniformReal(min_x,min_x);
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
	...
	{
	 float angle =atan2(particles.poses[i].orientation.z,particles.poses[i].orientation.w)*2;  //Creamos el angulo
	 particles.poses[i].position.x+=delta_x*cos(angle)-delta_y*sin(angle)+rnd.gaussian(0,MOVEMENT_NOISE); //Se mueven las particulas en X y Y
	 particles.poses[i].position.y+=delta_x*sin(angle)-delta_y*cos(angle)+rnd.gaussian(0,MOVEMENT_NOISE);  
