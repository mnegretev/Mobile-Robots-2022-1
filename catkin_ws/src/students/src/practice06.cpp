...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
     ...
     */
    //Generacion de partículas dando posicion y orientacion
    for(int i=0; i<N; i++){
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
    ...
    for(int i=0;i<particles.poses.size();i++){
      alpha=atan2(particles.poses[i].orientation.z,particles.poses[i].orientation.w)*2;

      particles.poses[i].position.x   +=  delta_x*cos(alpha) - delta_y*sin(alpha) + rnd.gaussian(0,MOVEMENT_NOISE);

      particles.poses[i].position.y   +=  delta_x*sin(alpha) + delta_y*cos(alpha) + rnd.gaussian(0,MOVEMENT_NOISE);
      
      alpha+= delta_t + rnd.gaussian(0,MOVEMENT_NOISE);

      particles.poses[i].orientation.z = sin(alpha/2);
      particles.poses[i].orientation.w = cos(alpha/2);
    }
...
int main(int argc, char** argv)
{
    ...
    while(ros::ok())
    {
        ...
        if(check_displacement(robot_odom, delta_pose))
        {
             ...
             * Check on
             line documentation of Pose2D message. 
             */
