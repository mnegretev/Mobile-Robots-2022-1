...
geometry_msgs::PoseArray get_initial_distribution(int N, float min_x, float max_x, float min_y, float max_y, float min_a, float max_a)
{
    ...
    float At = rnd.uniformReal(min_a, max_a);

    particles.poses[i].orientation.w  = cos(At/2);
    particles.poses[i].orientation.z  = sin(At/2);
...
void move_particles(geometry_msgs::PoseArray& particles, float delta_x, float delta_y, float delta_t)
{
    ...
    an += delta_t + rnd.gaussian(0, MOVEMENT_NOISE);

    particles.poses[i].orientation.w = cos(an/2);
    particles.poses[i].orientation.z = sin(an/2);
